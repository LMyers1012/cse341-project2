...
const getUserByDisplayName = async (req, res) => {
  ...
  try {
    ...
    if (!username) {
      res.status(400).send({ message: 'Invalid displayName Supplied' });
      return;
    }
    const userExists = await User.findOne({ displayName: username });
    // if (userExists) console.log('userExists');
    if (!userExists) {
      res.status(400).send({ message: 'This displayName does not exist.' });
      return;
    }
...
const deleteUserByGoogleId = async (req, res) => {
  // #swagger.tags = ['User']
  try {
    ...
    const userExists = await User.findOne({ googleId: userId });
    if (!userExists) {
      res
        .status(400)
        .send({ message: 'This Google ID does not exist in our system.' });
      return;
    }
    User.deleteOne({ googleId: userId }, function (err, result) {
      if (err) {
        res
          .status(500)
